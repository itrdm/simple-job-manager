<?php
class Rdm_Jobs_Settings_Option_Page{

	//Under which name to save the array of options
	static private $meta_name = 'rdm_job_plugin_options';
	...
	*/
	...
	static function list_pdf_templates(){
		...
		if (file_exists(TEMPLATEPATH . '/rdm_jobs/invoice_templates/template/')){
		...
		$saved_option = Rdm_Jobs_Settings_Option_Page::get('pdf_template');
	...
	*/
	...
	static function list_purchase_pdf_templates(){
		...
		$found_templates['template']['template3']= 'Default Template 3';
		$found_templates['template']['template4']= 'Default Template 4';
		

		//check if we are overriding from the theme folder
		if (file_exists(TEMPLATEPATH . '/rdm_jobs/purchase_templates/template/')){
			$found_templates['template']['theme']= 'Theme Template';
		}		
